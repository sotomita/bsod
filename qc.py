...
def get_qc_df(
    raw_df: pd.DataFrame, sonde_no: str, launch_timeUTC: datetime, **kwargs
) -> pd.DataFrame:
    ...
    for i in range(1, len(df)):
        ...
        time = time + timedelta(days=1) if time.hour < time0.hour else time
        df.loc[i, "Time"] = time
        time0 = time

    df["Time"] = df["Time"] - pd.to_timedelta(time_zone, unit="h")
